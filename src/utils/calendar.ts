 ...
 * 返回 日期 → 路径；同一天有多个时取路径最短的（直接放在 日记/ 下的那篇）。
 */
export function dailyNotesByDate(notes: { path: string }[], root: string): Map<string, string> {
  ...
  for (const { path } of notes) {
    ...
    if (!prev || path.length < prev.length) out.set(name, path);
  }
  return out;
}
